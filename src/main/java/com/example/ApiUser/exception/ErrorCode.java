...
public enum ErrorCode {
    ...
    USR_PASSWORD_NOT_FOUND(1003, "Password not found", HttpStatus.NOT_FOUND),
    USR_NOT_FOUND(1004, "User not found", HttpStatus.NOT_FOUND),
    USR_NAME_INVALID(1005, "Name must be at least {min} characters", HttpStatus.BAD_REQUEST),
    USR_USERNAME_INVALID(1006, "Username must be at least {min} characters", HttpStatus.BAD_REQUEST),
    USR_PASSWORD_INVALID(1007, "Password must be at least {min} characters", HttpStatus.BAD_REQUEST),
    USR_INVALID_DOB(1008, "Invalid date of birth (must be after {min})", HttpStatus.BAD_REQUEST),
    USR_AUTH_TOKEN_MISSING(1009, "Authentication token missing", HttpStatus.UNAUTHORIZED),
